...
public class HomeWork1{
  ...
  public void testHomePage() {
      ...
      //test user name
      Assert.assertEquals(driver.findElement(By.cssSelector(".profile-photo")).getText() , TestData.USERNAME);
      ...
      Assert.assertEquals(imagesList.size(),TestData.NUMBER_OF_IMAGES, "The number of images does not match the target.");
      imagesList.forEach(imgIterator->Assert.assertEquals(imgIterator.isDisplayed(), true, imgIterator.getAttribute("className")));
    
      //test 4 texts
      List<WebElement> texts = driver.findElements(By.cssSelector(".benefit-txt"));
      Assert.assertEquals(texts.size(), TestData.NUMBER_OF_TEXTS,
              String.format("The number of texts does not match the target.",texts.size(), TestData.NUMBER_OF_TEXTS));
      texts.forEach(textIterator-> Assert.assertTrue(TestData.BENEFITS_TEXTS.contains(textIterator.getText())));
