...
//    }
    ...
    public void testHomePage() {
       ...
       testPage.login(TestData.LOGIN, TestData.PASSWORD);
        ...
        Assert.assertEquals(testPage.getIconsNumber(),TestData.NUMBER_OF_IMAGES, "The number of images does not match the target.");

        //test 4 texts
        List<String> benefitsList = testPage.getBenefitTexts();
        
        Assert.assertEquals(benefitsList.size(), TestData.NUMBER_OF_TEXTS,
                String.format("The number of texts does not match the target.",benefitsList.size(), TestData.NUMBER_OF_TEXTS));
        benefitsList.forEach(textIterator-> Assert.assertTrue(TestData.BENEFITS_TEXTS.contains(textIterator.toString())));
